...
def acwr(
    ...
    if len(df) < min_periods:
        raise ValueError(
            f"Insufficient data: need at least {min_periods} days, got {len(df)}"
        )
...
def monotony(
    df: pd.DataFrame,
    window: int = DEFAULT_MONOTONY_WINDOW,
    latest_only: bool = True,
    min_periods: Optional[int] = None,
...
def get_workout_metrics(
    df: pd.DataFrame, return_series: bool = False
