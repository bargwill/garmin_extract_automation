...
def fetch_workouts(start_date: Optional[datetime] = None, 
...
def convert_to_dataframe(activities: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    for i, activity in enumerate(activities):
        try:
            ...
            activity_data = {
                "date": _parse_garmin_date(activity.get("startTimeLocal", activity.get("startTime"))),
                "distance": _safe_float(activity.get("distance", 0)) / 1000,  # Convert to km
                "duration": _safe_float(activity.get("duration", 0)) / 60,    # Convert to minutes
